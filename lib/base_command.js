...
class Command extends BaseCommand {
  ...
  async runHook(name, cacheKey) {
    ...
    let hook = this.config.hooks[name];
    const hookName = hook.split(' ')[0];
    const hooksDir = path.join(configDir, 'hooks');
    const env = {
      PATH: `${hooksDir}${path.delimiter}${process.env.PATH}`,
      PROJJ_HOOK_NAME: name,
    };
    const hookConfig = this.config[name] || this.config[hookName];
    if (hookConfig) {
      env.PROJJ_HOOK_CONFIG = JSON.stringify(hookConfig);
    }
    const opt = {
    ...
    if (process.platform === 'win32') {
      hook = await this.resolveHookScript(hook, hooksDir);
    }
  ...
  async changeDirectory(dir) {
    if (!this.config.change_directory) return false;
    ...
    this.logger.info('Auto change directory is not available on %s, fallback to clipboard', process.platform);
    return false;
  }

  async resolveHookScript(hook, hooksDir) {
    const parts = hook.split(' ');
    const hookPath = path.join(hooksDir, parts[0]);
    if (!(await fs.exists(hookPath))) return hook;
  ...
  // https://github.com/ying-bin/projj.git
  // => $BASE/github.com/ying-bin/projj
  url2dir(url) {
  ...
  async addRepo(repo, cacheKey) {
    ...
    await this.runScript(`git clone --progress ${repo} ${targetPath}`, {
      env,
    });
